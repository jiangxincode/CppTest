...
struct binary_node
{
	...
	binary_node *parent;
	binary_node() :
			data(0), count(1), left_child(NULL), right_child(NULL), parent(NULL)
	{
	}
	binary_node(int D, binary_node *L = NULL, binary_node *R = NULL,
			binary_node *P = NULL) :
...
public:
	...
	binary_tree(int ref_val) :
			root(NULL), ref_val(ref_val)
	{
	}
	~binary_tree()
	{
